...
maFun <- function(Data, experimentFactor=NULL, perGroup=FALSE, normMeth="", aType=NULL, 
  WIDTH=1000, HEIGHT=1414, MAXARRAY=41,gridsize=c(101,101)){                       
    ...
    nplots <- ceiling(length(sampleNames(x2))/nPerPage)
	  for(l in 1:nplots){     
        ...
        if(Type=="Raw" && aType!="PMMM") {
          MAplot(x2, pairs = FALSE, which= from:to, plot.method = "smoothScatter", lwd=3, type="pm", gridsize = gridsize, cex.main=cexmain,cex=cexval)    
        } else {
          MAplot(x2, pairs = FALSE, which= from:to, plot.method = "smoothScatter", lwd=3, gridsize = gridsize, cex.main=cexmain,cex=cexval)    
        }
